...
export function FindingsTable({ findings }: { findings: Finding[] }) {
  ...
  return (
    <div className="cyber-card overflow-hidden overflow-x-auto">
      <table className="w-full text-sm min-w-[720px]">
        <thead>
          <tr className="mono border-b border-border bg-white/[0.02] text-[11px] text-muted-foreground tracking-widest uppercase">
        ...
        <tbody>
          {sorted.map((f) => (
            <tr key={f.id} className="border-b border-border last:border-0 hover:bg-white/[0.025] transition-colors group">
              <td className="py-4 px-4">
                <span className="mono font-bold text-primary tracking-widest">{f.id}</span>
              </td>
              <td className="py-4 px-4">
                <span className={`mono text-[10px] font-bold tracking-widest uppercase px-2 py-1 rounded ${domainClass(f.domain)}`}>{f.domain}</span>
              </td>
              <td className="py-4 px-4 max-w-md">
                <Link href={`/leak/${f.id}`} className="block">
                  <div className="font-medium text-foreground group-hover:text-primary transition-colors">{f.title}</div>
                  <div className="text-xs text-muted-foreground line-clamp-1 mt-1">{f.summary}</div>
                </Link>
              </td>
              <td className="py-4 px-4">
                <span className="text-xs mono text-muted-foreground">{f.service || '—'}</span>
              </td>
              <td className="py-4 px-4 text-right">
                <span className="text-sm font-bold mono neon-green">{moneyMo(f.money?.cost_month)}</span>
              </td>
